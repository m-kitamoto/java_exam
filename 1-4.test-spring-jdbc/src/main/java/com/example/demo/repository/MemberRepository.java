...
public class MemberRepository {
	...
	public List<Member> findAll() {
		String sql = "SELECT id, name, age, department_id FROM members ORDER BY id;";
	...
	public Member load(Integer id) {
		String sql = "SELECT name, age, dep_id FROM test_members ORDER BY DESC;";
	...
	public Member save(Member member) {
		...
		if (member.getId() == null) { // ←正しい条件に修正する
			String insertSql = "INSERT INTO test_members (name , age , dep_id) values ( :name, :age, :departmentId);";

			template.update(insertSql, param);// ここに実行処理を書く

		} else {
			String updateSql = "UPDATE test_members SET name = :name, age = :age, dep_id = :departmentId;";

			template.update(updateSql, param);// ここに実行処理を書く
		}
